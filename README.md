...
> Server = サービスを提供するもの。ハイジでいうセバスチャン（執事）のようなイメージ

## 1.1 ハードウェアとしてのサーバ

サーバという言葉は、コンピュータの筐体を意味します。サーバコンピュータには、ディスプレイやキーボード、マウスなどを接続しないことが多く、リモートのマシンからログインして操作することもあります。


### クラウド

2000年頃まで、サーバといえば、大金を払って購入し、自社のサーバルームや、データセンターに配置するのが一般的でした。

最近では、インターネットの高速化、ハードウェアの小型化、仮想化技術の進化など、様々なテクノロジーが融合し、クラウドと呼ばれる形態でサーバを利用できるようになりました。
...
普段、ヨドバシなどの量販店ではなかなか見ることがありませんがWindowsには「Windows Server2012」など、サーバ用途に特化したOSもあります。
